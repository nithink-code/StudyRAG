...
def tab_upload():
    ...
    <div class="animated-border-box">
        <div class="animated-border-box-content">
            <h3 style="margin-top:0;">📂 Upload Knowledge Base</h3>
    ...
    with col2:
        ...
        if uploaded is not None:
            if pypdf is None:
                ...
                if st.button("🚀 Process System Initiation", use_container_width=True):
                    with st.status("Analyzing document structure...", expanded=True) as status:
                        try:
                            st.write("📖 Parsing binary data...")
                            reader = pypdf.PdfReader(uploaded)
                            text = ""
                            for page in reader.pages:
                                text += page.extract_text() + "\n"
                            
                            st.write("📤 Synchronizing with Neural Network...")
                            asyncio.run(send_rag_ingest_event(text, uploaded.name))
                            
                            status.update(label="✅ Integration Complete!", state="complete", expanded=False)
...
def tab_chat():
    ...
    <div class="animated-border-box">
        <div class="animated-border-box-content">
            <h3 style="margin-top:0;">💬 Neural Interface</h3>
    ...
    with st.expander("⚙️ Retrieval Configuration"):
...
def main():
    ...
    with col1:
        if st.button("📂 DATA INGESTION", use_container_width=True, type="primary" if st.session_state["active_tab"] == "upload" else "secondary"):
            st.session_state["active_tab"] = "upload"
            st.rerun()
    with col2:
        if st.button("💬 NEURAL CHAT", use_container_width=True, type="primary" if st.session_state["active_tab"] == "chat" else "secondary"):
            st.session_state["active_tab"] = "chat"
            st.rerun()
